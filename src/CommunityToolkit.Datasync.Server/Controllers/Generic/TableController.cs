...
public partial class TableController<TEntity, TKey> : ODataController where TEntity : class, ITableData<TKey> where TKey : IParsable<TKey>, IEquatable<TKey>
{
    ...
    public TableController()
        : this(new Repository<TEntity, TKey>(), new AccessControlProvider<TEntity, TKey>(), null,
    ...
    public TableController(IRepository<TEntity, TKey> repository)
        : this(repository, new AccessControlProvider<TEntity, TKey>(), null, new TableControllerOptions())
    {
    }
    ...
    public TableController(IRepository<TEntity, TKey> repository, IEdmModel model)
        : this(repository, new AccessControlProvider<TEntity, TKey>(), model, new TableControllerOptions())
    {
    }
    ...
    public TableController(IRepository<TEntity, TKey> repository,
        IAccessControlProvider<TEntity, TKey> accessControlProvider)
    ...
    public TableController(IRepository<TEntity, TKey> repository, TableControllerOptions options)
        : this(repository, new AccessControlProvider<TEntity, TKey>(), null, options)
    {
    }
    ...
    /// <param name="options">The <see cref="TableControllerOptions"/> to use for configuring this controller.</param>
    public TableController(IRepository<TEntity, TKey> repository,
        IAccessControlProvider<TEntity, TKey> accessControlProvider, TableControllerOptions options)
    ...
    /// <param name="model">The <see cref="IEdmModel"/> to use for OData interactions.</param>
    public TableController(IRepository<TEntity, TKey> repository,
        IAccessControlProvider<TEntity, TKey> accessControlProvider, IEdmModel model)
    ...
    public TableController(IRepository<TEntity, TKey> repository,
        IAccessControlProvider<TEntity, TKey> accessControlProvider, IEdmModel? model, TableControllerOptions options)
    {
        ...
        if (EdmModel.FindType(typeof(TEntity).FullName) is null)
        {
            throw new InvalidOperationException(
                $"The entity type {typeof(TEntity).FullName} is not registered in the OData EdmModel.");
        }
    }
    ...
    /// </summary>
    public IAccessControlProvider<TEntity, TKey> AccessControlProvider { get; set; }
    ...
    [NonAction]
    protected virtual async ValueTask AuthorizeRequestAsync(TableOperation operation, TEntity? entity,
        CancellationToken cancellationToken = default)
    {
        bool isAuthorized = await AccessControlProvider.IsAuthorizedAsync(operation, entity, cancellationToken)
            .ConfigureAwait(false);
        if (!isAuthorized)
        {
            Logger.LogWarning("{operation} {entity} statusCode=401 unauthorized", operation,
                entity?.ToJsonString() ?? "");
    ...
    [NonAction]
    protected virtual ValueTask PostCommitHookAsync(TableOperation operation, TEntity entity,
        CancellationToken cancellationToken = default)
    {
    ...
    {
        IDatasyncServiceOptions options = HttpContext.RequestServices?.GetService<IDatasyncServiceOptions>() ??
                                          new DatasyncServiceOptions();
        HttpContext.Request.EnableBuffering();
        if (HttpContext.Request.HasJsonContentType())
        {
            TEntity entity = await JsonSerializer
                                 .DeserializeAsync<TEntity>(HttpContext.Request.Body, options.JsonSerializerOptions,
            ...
            {
                throw new HttpException(StatusCodes.Status400BadRequest, "Invalid entity")
                {
    ...
    protected static TKey ParseId(string id)
    {
        if (TKey.TryParse(id, CultureInfo.CurrentCulture, out TKey? parsed))
        {
            return parsed;
        }
        
        throw new HttpException(StatusCodes.Status400BadRequest, "Badly formatted ID");
    }
}